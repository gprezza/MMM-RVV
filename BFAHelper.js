...
module.exports = {
	...
	// This might help you to check, if MMM-BFA is fetching the correct data
	...
	addTrip: function(tripObj, trips, config){
		...
		if (trips.length >= config.maximumTripsToShow) {
			this.printToConsole("MMM-BFA could fetch more trips, but they were limited to " + config.maximumTripsToShow + " trips. Aborting the fetch.", config);
			limitReached = true;
		}
		else {
			trips.push(tripObj);
